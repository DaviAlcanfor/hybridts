...
def test_train_test_split(sample_timeseries):
    ...
    df_train, df_test = processor.df_train_test_split(sample_timeseries, split_size=30)
...
def test_preparar_dados_completos_basic(sample_timeseries):
    result = TimeSeriesProcessor().preparar_dados_completos(df=sample_timeseries)
    
    assert list(result.columns) == ["ds", "y"]
    assert len(result) == len(sample_timeseries)


def test_preparar_dados_completos_fills_gaps(df_with_gaps):
    result = TimeSeriesProcessor().preparar_dados_completos(df=df_with_gaps)
...
def test_preparar_dados_completos_rejects_nulls():
    df = pd.DataFrame({"ds": pd.date_range("2023-01-01", periods=5), "y": [1, None, 3, 4, 5]})
    with pytest.raises(ValueError, match="NULL"):
        TimeSeriesProcessor().preparar_dados_completos(df=df)


def test_preparar_dados_completos_rejects_negatives():
    df = pd.DataFrame({"ds": pd.date_range("2023-01-01", periods=5), "y": [1, -2, 3, 4, 5]})
    with pytest.raises(ValueError, match="negativos"):
        TimeSeriesProcessor().preparar_dados_completos(df=df)


def test_preparar_dados_completos_rejects_wrong_columns():
    df = pd.DataFrame({"date": pd.date_range("2023-01-01", periods=5), "value": range(5)})
    with pytest.raises(ValueError, match="'ds'"):
        TimeSeriesProcessor().preparar_dados_completos(df=df)


def test_preparar_dados_completos_with_data_loader(sample_timeseries, tmp_path):
    csv = tmp_path / "data.csv"
    sample_timeseries.to_csv(csv, index=False)
    result = TimeSeriesProcessor().preparar_dados_completos(data_loader=lambda: pd.read_csv(csv, parse_dates=["ds"]))
    
    assert len(result) == len(sample_timeseries)


def test_preparar_dados_completos_requires_input():
    with pytest.raises(ValueError, match="Forneça"):
        TimeSeriesProcessor().preparar_dados_completos()
