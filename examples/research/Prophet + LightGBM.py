# NOTA: dbutils removido - para Databricks descomente: dbutils.library.restartPython()
# Instalação local: pip install -U sktime xgboost prophet holidays pandas numpy lightgbm
...
residuos_finais = hybrid_forecast_lgbm_cv - y_test.astype(float)
lb_prophet = acorr_ljungbox(residuos_finais.dropna(), lags=[7, 14], return_df=True)
print("\n", lb_prophet)


# df_prev_fev = hybrid_forecast_lgbm_cv.to_frame(name="yhat")
...
# df_test_filtrado = df_test[df_test.index.month == 2]
# sum(df_test_filtrado['y'])
