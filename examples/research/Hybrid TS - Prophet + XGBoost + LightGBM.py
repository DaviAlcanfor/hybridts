...
# NOTA: dbutils foi removido para uso fora do Databricks
# Se estiver no Databricks, descomente a linha abaixo:
# dbutils.library.restartPython()

# Para uso local, instale as dependências manualmente:
# pip install -U sktime xgboost prophet holidays pandas numpy lightgbm
...
# NOTA: SparkSession removido - não é mais necessário              
...
# **Para uso fora do Databricks**, forneça seus dados como CSV/Parquet com as colunas:
# - `ds`: data da observação (formato YYYY-MM-DD)
# - `y`: valor do TPV (numérico)
...
# OPÇÃO 1: Carregar de arquivo CSV local ou remoto
df = pd.read_csv('seu_arquivo_tpv.csv', parse_dates=['ds'])
# df deve ter colunas: ['ds', 'y']

# OPÇÃO 2: Carregar de Parquet
# df = pd.read_parquet('seu_arquivo_tpv.parquet')

# OPÇÃO 3 (LEGACY): Se estiver usando Databricks, descomente abaixo
...
#     AND date(reference_date) > date('2023-01-01')
# group by all
# ORDER BY 1 
# """
...
df_prev_fev2 = hybrid_forecast.to_frame(name="yhat")
df_prev_fev2['yhat'].astype(float)
sum(df_prev_fev2['yhat'])
...
plt.title("O Futuro do TPV: Previsão Híbrida Final")
plt.legend()
...
plot_series(df_march_2025, title="TPV em Março de 2025")
plot_series(df_march_2024, title="TPV em Março de 2024")
plot_series(df_march_2023, title="TPV em Março de 2023")
...
df_prev_fev2 = hybrid_forecast.to_frame(name="yhat")
df_prev_fev2['yhat'].astype(float)
df_filtrado = df_prev_fev2[df_prev_fev2.index.month == 2]
sum(df_filtrado['yhat'])
...
df_prev_fev_lgbm = hybrid_forecast_lgbm_cv.to_frame(name="yhat")
df_prev_fev_lgbm['yhat'].astype(float)
df_filtrado = df_prev_fev_lgbm[df_prev_fev_lgbm.index.month == 2]
sum(df_filtrado['yhat'])
...
df_res_filtrado = df_resultado[df_resultado['Data'].dt.month == 3]

sum(df_res_filtrado['Previsao_TPV'])
...
df_march_2025_f = df_march_2025.reset_index(drop=True)
df_march_2024_f = df_march_2024.reset_index(drop=True)
df_march_2023_f =df_march_2023.reset_index(drop=True)
plot_series(df_march_2023_f, df_march_2024_f, df_march_2025_f, 
            labels=["2023", "2024", "2025"],
            title="TPV em Março de 2025")


total_marco_2023 = sum(df_march_2023['y'])
total_marco_2024 = sum(df_march_2024['y'])
total_marco_2025 = sum(df_march_2025['y'])

print(f"Total TPV Março 2023: {total_marco_2023}")
print(f"Total TPV Março 2024: {total_marco_2024}")
print(f"Total TPV Março 2025: {total_marco_2025}")